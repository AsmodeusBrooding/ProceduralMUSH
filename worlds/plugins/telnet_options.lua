...
local function TelnetOption (which, on)
  ...
  -- Telnet subnegotiation for Procedural Realms
  local PROCEDURAL_TELOPT = 102
  ...
  if on then
    SendPkt (string.char (IAC, SB, PROCEDURAL_TELOPT, which, TELOPT_ON, IAC, SE)) 
  else
    SendPkt (string.char (IAC, SB, PROCEDURAL_TELOPT, which, TELOPT_OFF, IAC, SE)) 
  end -- if
